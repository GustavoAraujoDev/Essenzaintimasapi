...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  await Promise.all([loadProducts(), initTables()]);
});
...
async function scanNetworkPrinters() {
  ...
  try {
    const response = await fetch("http://127.0.0.1:3000/pedidos/discover");
...
async function selectAndConnectPrinter(ip, name) {
  ...
  try {
    const response = await fetch("http://127.0.0.1:3000/pedidos/connect", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function testPrint() {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:3000/pedidos/test-print`, {
      method: "POST",
    });
    const result = await response.json();
