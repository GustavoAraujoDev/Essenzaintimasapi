...
class EpsonPrinterService {
  ...
  connectToPrinter() {
    try {
      this.device = new escpos.USB(); // Pega a primeira impressora USB conectada
      this.printer = new escpos.Printer(this.device, { encoding: "CP860" });
      this.isMock = false;
      console.log("[PRINTER] Conectado com sucesso via cabo USB.");
      return { success: true };
    } catch (err) {
      this.isMock = true;
      console.error(
        "[PRINTER_CONN_ERROR] Nenhuma impressora USB achada. Usando Modo Mock.",
        err.message,
      );
